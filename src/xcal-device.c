...
//	int result = getipaddress(devConf->bcastIf, ipAddressBuffer);
    ...
    context = gupnp_context_new (NULL, devConf->bcastIf, devConf->bcastPort, &error);
    if (error) {
    ...
    /* Create root device */
    dev = gupnp_root_device_new (context, devConf->devXmlFile, devConf->devXmlPath);
...
gboolean readconffile(const char* configfile)
{
    ...
    */
    ...
    devConf->wbFile = g_key_file_get_string             (keyfile, "DataFiles","WbFile", NULL);
...
{
    if ((service_ready==FALSE) || (!service))
    {
